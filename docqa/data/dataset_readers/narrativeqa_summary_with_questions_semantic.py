...
def crop_semantic_views(semantic_views, crop_start, crop_end, axis=-1):
    """
    ...
    def crop_single_view(arr, crop_start, crop_end, axis=-1):
        if axis == -1:
    ...
    if isinstance(semantic_views, tuple):
        return [crop_single_view(x, crop_start, crop_end, axis=axis) for x in semantic_views]
    elif isinstance(semantic_views, Dict):
        return {k: crop_single_view(v, crop_start, crop_end, axis=axis) for k, v in semantic_views.items()}
    elif isinstance(semantic_views, np.ndarray):
        return crop_single_view(semantic_views, crop_start, crop_end, axis=axis)
    else:
        raise ValueError("Unexpected type!")


@DatasetReader.register("narrativeqa_summary_with_questions_semantic")
class NarrativeQASummaryWithQuestionsSemanticsReader(DatasetReader):
    """
    ...
    def __init__(self,
                 ...
                 srl_views_extractor: SRLInteractionViews = None
                 ) -> None:
        super().__init__(lazy)
        self._tokenizer = tokenizer or WordTokenizer()
        self._token_indexers = token_indexers or {'tokens': SingleIdTokenIndexer()}
        self._srl_views_extractor = srl_views_extractor
    ...
    def _read(self, file_path: str):
        ...
        with open(file_path) as dataset_file:
            ...
            for line_id, line in enumerate(dataset_file):
                ...
                if self._srl_views_extractor is not None:
                    paragraph_semantic_views = self._srl_views_extractor.extract_features(paragraph_json["summary_parse"])
                ...
                for q_id, question_answer in enumerate(paragraph_json['questions']):
                    ...
                    if self._srl_views_extractor is not None:
                        ...
                        question_semantic_views = self._srl_views_extractor.extract_features(question_answer["question_parse"])
    ...
    def text_to_instance(self,  # type: ignore
        ...
        if self.passage_length_limit is not None and self.passage_length_limit < len(passage_tokens):
            ...
            assert(token_spans[0][1] < len(passage_tokens))
