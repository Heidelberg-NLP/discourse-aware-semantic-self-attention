...
from docqa.allennlp_custom.modules.seq2seq_encoders.multi_head_semantic_flat_concat_self_attention import \
    semantic_integration_mode_supported
...
@Seq2SeqEncoder.register("qanet_semantic_flat_concat_encoder")
class QaNetSemanticFlatConcatEncoder(Seq2SeqEncoder):
    """
    ...
    def __init__(self,
                 ...
                 semantic_integration_mode: str,
                 use_positional_encoding: bool = True,
                 semantic_emb_dim: int = 0,  # this is only used with specific semantic_integration_mode
        ...
        self._encoder_blocks: List[QaNetSemanticFlatConcatEncoderBlock] = []
        for block_index in range(num_blocks):
            if self.use_semantic_views and block_index in self.semantic_views_layers:
                encoder_block = QaNetSemanticFlatConcatEncoderBlock(hidden_dim,
                                                                    hidden_dim,
    ...
    def forward(self, inputs: torch.Tensor,
                ...
                return_output_metadata: bool = False) -> torch.Tensor:  # pylint: disable=arguments-differ
        inputs = self._input_projection_layer(inputs)
        output = inputs

        encoder_blocks_output_meta = {}
        for encoder_block_id, encoder_block in enumerate(self._encoder_blocks):
            if self.use_semantic_views and (isinstance(encoder_block, QaNetSemanticFlatConcatEncoderBlock)\
               or isinstance(encoder_block, QaNetSemanticEncoderBlock)):
                if is_output_meta_supported(encoder_block):
                    output, output_meta = encoder_block(output,
                                           semantic_views_q,
                                           semantic_views_k,
                                           mask,
                                           return_output_metadata)
                    encoder_blocks_output_meta["encoder_block_{0:03d}".format(encoder_block_id)] = output_meta
                else:
                ...
                output = encoder_block(output,
                                       mask)
        return output, encoder_blocks_output_meta


@Seq2SeqEncoder.register("qanet_semantic_flat_concat_encoder_block")
class QaNetSemanticFlatConcatEncoderBlock(Seq2SeqEncoder):
    """
    ...
    def __init__(self,
        ...
        self.attention_layer = MultiHeadSemanticFlatConcatSelfAttention(num_heads=num_attention_heads,
    ...
    def forward(self,
        ...
        if is_output_meta_supported(self.attention_layer):
            attention_out, attention_output_meta = self.attention_layer(attention_norm_out,
                                                              semantic_views_q,
