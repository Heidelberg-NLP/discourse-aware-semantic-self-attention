...
@SimilarityFunction.register("linear_transform_sum_tri")
class LinearTransformSumReprCombinationTriParams(SimilarityFunction):
    """
    ...
    def __init__(self,
        ...
        self.weight_tensor3 = Parameter(torch.Tensor(tensor3_dim, output_dim))
        self.bias = Parameter(torch.Tensor(output_dim))

        if output_dim == 0:
            if tensor1_dim != tensor2_dim or tensor1_dim != tensor3_dim:
                raise ValueError("tensor1_dim and tensor2_dim and tensor3_dim should be equal for this function")
    ...
    def forward(self, tensor1:torch.LongTensor, tensor2:torch.LongTensor, tensor3:torch.LongTensor) -> torch.Tensor:
        ...
        res_repr = torch.matmul(tensor1, self.weight_tensor1) \
                   ...
                   + torch.matmul(tensor3, self.weight_tensor3) \
                   + self.bias
