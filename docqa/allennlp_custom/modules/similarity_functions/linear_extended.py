...
from docqa.allennlp_custom.nn.util import get_combined_dim, combine_tensors


@SimilarityFunction.register("linear_extended")
class LinearExtenedSimilarity(SimilarityFunction):
    """
    This similarity function performs a linear transformation over
    a combination of the two input vectors, followed by an (optional) activation function.  The
    combination used is configurable.

    The difference with the original ``linear`` similarity function is that it allows some function over a combination
    argument such as abs(x-y), halve(x-y), etc.

    If the two vectors are ``x`` and ``y``, we allow the following kinds of combinations: ``x``,
    ``y``, ``x*y``, ``x+y``, ``abs(x-y)``, ``x/y``, where each of those binary operations is performed
    elementwise.  You can list as many combinations as you want, comma separated.  For example, you
    might give ``x,y,x*y,abs(x-y)`` as the ``combination`` parameter to this class.  The computed similarity
